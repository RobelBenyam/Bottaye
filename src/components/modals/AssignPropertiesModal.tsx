...
export default function AssignPropertiesModal({ user, onClose, onSuccess }: AssignPropertiesModalProps) {
  const [loading, setLoading] = useState(false)
  const [properties, setProperties] = useState<Property[]>([])
  const [selectedPropertyIds, setSelectedPropertyIds] = useState<string[]>(user.propertyIds || [])
      ...
      <div className="bg-white rounded-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-secondary-900">Assign Properties</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="font-medium text-secondary-900">{property.name}</h4>
                          <p className="text-sm text-secondary-600 truncate">{property.address}</p>
                        </div>
          ...
          <div className="flex gap-3 pt-4 border-t">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? 'Saving...' : `Assign ${selectedPropertyIds.length} ${selectedPropertyIds.length === 1 ? 'Property' : 'Properties'}`}
            </button>
          </div>
        </form>
      </div>
