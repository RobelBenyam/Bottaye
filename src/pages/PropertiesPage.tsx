...
import { Plus, Search, MapPin, Building2, Edit3, Camera, Image as ImageIcon } from 'lucide-react'
...
export default function PropertiesPage() {
  ...
  const loadProperties = async () => {
    try {
      ...
      try {
        ...
        getUnits = localUnitService.getByPropertyId;
      }
      ...
      const propertiesWithStats = await Promise.all(
        propertiesData.map(async (property) => {
          ...
          const monthlyRevenue = units.reduce((sum, unit) => sum + (unit.rent || 0), 0)
  ...
  const handleEditProperty = (property: PropertyWithStats, e: React.MouseEvent) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h1 className="text-3xl font-bold text-secondary-900 dark:text-secondary-100">Properties</h1>
          <p className="text-secondary-600 dark:text-secondary-400 mt-1">Manage your property portfolio and track performance.</p>
        </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <div className="text-lg text-secondary-600 dark:text-secondary-400">Loading properties...</div>
        </div>
      ) : properties.length === 0 ? (
        <InitializeDatabase />
      ) : (
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {properties.map((property) => (
              <div 
                ...
                onClick={() => navigate(`/units?propertyId=${property.id}&propertyName=${encodeURIComponent(property.name)}`)}
              >
                {/* Property Image */}
                <div className="relative h-48 mb-4 rounded-lg overflow-hidden bg-secondary-100 dark:bg-secondary-700">
                  ...
                  <div className="absolute bottom-2 left-2">
                    <span className={`px-3 py-1 text-sm font-medium rounded-full ${
                      property.type === 'residential' ? 'bg-primary-100 text-primary-700' :
                      property.type === 'commercial' ? 'bg-warning-100 text-warning-700' :
                ...
                <div className="px-1">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-secondary-900 dark:text-secondary-100 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">{property.name}</h3>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-secondary-900 dark:text-secondary-100">{property.totalUnits}</p>
                  </div>
                  <div>
                    <p className="text-sm text-secondary-600 dark:text-secondary-400">Occupied</p>
                    <p className="text-lg font-semibold text-success-600">{property.occupiedUnits}</p>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-secondary-200 dark:border-secondary-600">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="text-right">
                      <p className="text-sm text-secondary-600 dark:text-secondary-400">Occupancy</p>
