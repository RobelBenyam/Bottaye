...
import { ArrowLeft, Edit3, Camera, FileText, User, Calendar, Phone, Mail, MapPin } from 'lucide-react'
import { Unit, Property } from '../types'
import { unitService, propertyService } from '../services/database'
import { localUnitService, localPropertyService } from '../services/localStorage'
    ...
    const loadUnitData = async () => {
      ...
      try {
        ...
        try {
          ...
          if (unitData) {
            propertyData = await localPropertyService.getById(unitData.propertyId)
          }
        }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-lg text-secondary-600 dark:text-secondary-400">Loading unit...</div>
      </div>
    )
  }
  ...
  if (!unit) {
    return (
      <div className="text-center py-12">
        <div className="text-lg text-secondary-900 dark:text-secondary-100 mb-2">Unit not found</div>
        <button 
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-secondary-900 dark:text-secondary-100">Unit {unit.unitNumber}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-secondary-800 rounded-xl shadow-sm border border-secondary-200 dark:border-secondary-700 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-secondary-900 dark:text-secondary-100">Unit Photos</h2>
            ...
            {unit.images && unit.images.length > 0 ? (
              ...
              <div className="border-2 border-dashed border-secondary-300 dark:border-secondary-600 rounded-lg p-8 text-center">
                ...
                <p className="text-secondary-600 dark:text-secondary-400 mb-2">No photos uploaded yet</p>
          ...
          <div className="bg-white dark:bg-secondary-800 rounded-xl shadow-sm border border-secondary-200 dark:border-secondary-700 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-secondary-900 dark:text-secondary-100">Floor Plan</h2>
              <button className="btn-secondary flex items-center text-sm">
            ...
            {unit.floorPlan ? (
              ...
              <div className="border-2 border-dashed border-secondary-300 dark:border-secondary-600 rounded-lg p-8 text-center">
                ...
                <p className="text-secondary-600 dark:text-secondary-400 mb-2">No floor plan uploaded yet</p>
        ...
        <div className="space-y-6">
          {/* Unit Info */}
          <div className="bg-white dark:bg-secondary-800 rounded-xl shadow-sm border border-secondary-200 dark:border-secondary-700 p-6">
            <h3 className="text-lg font-semibold text-secondary-900 dark:text-secondary-100 mb-4">Unit Information</h3>
            <div className="space-y-4">
              <div>
                <label className="text-sm text-secondary-600 dark:text-secondary-400">Type</label>
                <p className="font-medium text-secondary-900 dark:text-secondary-100">{unit.type.replace('_', ' ')}</p>
              </div>
              <div>
                <label className="text-sm text-secondary-600 dark:text-secondary-400">Monthly Rent</label>
                <p className="font-medium text-secondary-900 dark:text-secondary-100">{formatCurrency(unit.rent)}</p>
              </div>
              <div>
                <label className="text-sm text-secondary-600 dark:text-secondary-400">Security Deposit</label>
                <p className="font-medium text-secondary-900 dark:text-secondary-100">{formatCurrency(unit.deposit)}</p>
              </div>
              <div>
                ...
                <p className={`font-medium capitalize ${
                  ...
                  unit.status === 'vacant' ? 'text-warning-600' :
                  'text-error-600'
                }`}>
                  {unit.status}
                </p>
          ...
          {unit.status === 'occupied' && unit.tenantId && (
            <div className="bg-white dark:bg-secondary-800 rounded-xl shadow-sm border border-secondary-200 dark:border-secondary-700 p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-secondary-900 dark:text-secondary-100">Current Tenant</h3>
                <button 
              ...
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <p className="font-medium text-secondary-900 dark:text-secondary-100">{unit.tenantName}</p>
                  <p className="text-sm text-secondary-600 dark:text-secondary-400">Tenant ID: {unit.tenantId}</p>
                </div>
              </div>
              <div className="space-y-3 text-sm">
                <div className="flex items-center space-x-3">
                  <Calendar className="h-4 w-4 text-secondary-400" />
                  <span className="text-secondary-600 dark:text-secondary-400">Move-in date: Coming soon</span>
                </div>
                <div className="flex items-center space-x-3">
                  <Phone className="h-4 w-4 text-secondary-400" />
                  <span className="text-secondary-600 dark:text-secondary-400">Contact: Coming soon</span>
                </div>
              </div>
            </div>
          )}
